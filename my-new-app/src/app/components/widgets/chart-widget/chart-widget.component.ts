  ...
  template: `
    <div class="widget-card fade-in">
      ...
      <div class="widget-content">
        ...
        <div class="chart-legend" *ngIf="authorData.length > 0">
          <div
            *ngFor="let item of authorData; let i = index"
...
export class ChartWidgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.isBrowser) {
      this.bookService.getBooksByAuthor().subscribe((data) => {
        console.log('Fetched author data:', data); // Debugging
  ...
  createChartIfReady() {
    if (!this.chartReady || !this.chartCanvas || !this.authorData.length) return;
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (context) => {
                const percentage = (
                  (context.parsed / this.totalBooks) *
                  100
                ).toFixed(1);
